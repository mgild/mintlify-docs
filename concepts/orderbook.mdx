...
flowchart LR
    subgraph Traditional["Traditional"]
        T1["ALL ORDERS<br/>hits size limits<br/>slow to search"]
    end

    Traditional -->|Sharding| Sharded

    subgraph Sharded["Sharded"]
        S0["Shard 0<br/>$0-$100"]
        S1["Shard 1<br/>$100-$200"]
        S2["Shard 2<br/>$200-$300"]
        S0 <--> S1 <--> S2
    end
...
| 25% | Shard sparse, neighbor sparse | Merge | Prevent sparseness |
...
| Limit order | 88 bytes | Including tree node and density curve |
| CLMM order | 96 bytes | Including tree node and density curve |
