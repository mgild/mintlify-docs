...
flowchart LR
    ...
    subgraph Engine["MATCHING ENGINE"]
        E1["1. Find best prices"]
        E2["2. Fill orders"]
        E3["3. Transfer tokens"]
        E1 --> E2 --> E3
    end
...
const ix = Market.swapIx({
  ...
  limitPrice: 0n,              // No price limit (market order)
...
interface TakeParams {
  ...
  limitPrice: bigint;    // Max price you'll pay (0 = no limit)
  side: Side;            // Ask = buying base, Bid = selling base
  minOrderBips: number;  // Skip tiny orders (saves compute)
  maxOrders: number;     // Max orders to fill (saves compute)
}
```
...
| `limitPrice` | Stop filling above/below this | `155_000_000n` = $155 max |
| `side` | Direction of trade | `Ask` = buy, `Bid` = sell |
| `minOrderBips` | Skip orders smaller than X% | `100` = skip orders &lt;1% |
| `maxOrders` | Stop after N fills | `10` = max 10 orders |
...
INPUT: amount, minOut, limitPrice, side, maxOrders
...
WHILE amount_remaining > 0 AND orders_touched < maxOrders:
    ...
    IF limitPrice > 0 AND violatesLimit(best.price, limitPrice, side):
        BREAK  // Hit price limit
...
| `MAX_SKIPS` (32) | Don't waste CU on tiny orders |
...
| Mechanism | Purpose |
|-----------|---------|
| `limitPrice` | Stop if price exceeds limit |
...
With minOrderBips = 100 (1%):
  ...
  After 32 skips: Stop trying small orders
  Fill from real orders
```
...
fn violates_limit(order_price: u64, limit: u64, side: Side) -> bool {
    if limit == 0 { return false; } // No limit set

    match side {
        // For asks: order price must be <= limit
        Side::Ask => order_price > limit,

        // For bids: order price must be >= limit
        Side::Bid => order_price < limit,
    }
}
```
...
<Accordion title="Compute Unit Breakdown">

Typical CU costs for matching operations:

| Operation | CU Cost | Notes |
|-----------|---------|-------|
| Load market | ~3,000 | Account deserialization |
| Load shard | ~5,000 | Per shard loaded |
| Find best price | ~1,000 | Per source queried |
| Fill limit order | ~8,000 | Tree update included |
| Fill CLMM order | ~12,000 | Includes flip to inverse |
| Fill PropAMM order | ~8,000 | Similar to limit |
| Token transfer | ~3,000 | Per transfer (2 per swap) |

### Transaction Budget

A typical swap uses:
- Market load: 3,000 CU
- 6 shards: 30,000 CU
- Best price (3 sources): 3,000 CU
- 5 fills: 40,000-60,000 CU
- Transfers: 6,000 CU

**Total: ~90,000 CU** (well under 1.4M limit)
