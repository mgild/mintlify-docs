...
graph TB
    subgraph Defenses["BRAID MEV DEFENSES"]
        ...
        L3["Layer 3: SLIPPAGE<br/>min_out reverts<br/>Auto price bounds<br/>Limits profit margins"]
        L4["Layer 4: HARDENING<br/>Sequence numbers<br/>Capacity limits<br/>Minimum order sizes"]
        DefensesSpacer --> L1 --> L2 --> L3 --> L4
    end

    style DefensesSpacer fill:transparent,stroke:transparent,color:transparent
...
PropAMM orders are designed for competitive spreads through zero fees and slot-scoping. The short validity window also happens to provide strong MEV resistance.

### How Slot-Scoping Works
...
Timeline of a PropAMM order:
  ...
  Slot N-1        Slot N           Slot N+1
     ...
     │              │                 │
     │              │   Valid ~400ms  │
     │              │   (zero fees,   ├── Order EXPIRED
     │              │   tight spread) │   (unfillable)
     │              │                 │
     ▼              ▼                 ▼

Why this helps MEV resistance:
  1. Bot sees your tx in mempool
  2. Analyzes profitability
  3. Constructs attack tx
  4. Too slow - order already expired
...
This provides "all or nothing" protection - if the trade isn't good enough, it doesn't happen. The matching engine automatically enforces a price limit of 20 bips from the estimated output, preventing significant price manipulation.
...
4. Tiny orders skipped automatically
