...
Think of sharding like a library organizing books:

- **Without sharding**: All books in one giant pile. Finding "Programming in Rust" means searching through everything.
- **With sharding**: Books organized by category. Programming books are in Section C, shelves 10-15. You only search there.

Braid's shards are like library sections - each handles a specific "category" (price range) of orders.
...
### Step 1: Find the Right Shards

The SDK identifies which shards contain prices near the current market:

```typescript
// The SDK handles this automatically
const shards = await market.getShardsForSwap(side, amount);
// Returns shards along the appropriate chain (bid chain for sells, ask chain for buys)
```
...
Orders moved: High-price orders migrate to new shard
Rent paid: Someone pays ~0.01 SOL to create new account
...
flowchart LR
    ...
    subgraph After["After (Shard 1 deleted,<br/>rent reclaimed!)"]
...
flowchart LR
    subgraph Fill["Fill Level 0% ────────────────────────────────────── 100%"]
        direction LR
...
| Instruction | When to Call | Reward |
|-------------|--------------|--------|
| `CrankSplit` | Shard >75% full | Rent subsidy |
| `CrankMerge` | Adjacent shards &lt;25% each | Rent reclaim |
| `CrankRebalance` | One shard &lt;25%, neighbor has room | None |

### Example: Calling CrankSplit

```typescript
import { crankSplit } from '@braid/client';

// Anyone can call this - it's permissionless
const tx = await crankSplit({
  market: marketAddress,
  sourceShard: overloadedShardAddress,
  // New shard will be created automatically
});
...
- **Split**: Cranker pays ~0.01 SOL rent for new account
- **Merge**: Cranker receives ~0.01 SOL rent from closed account
- **Rebalance**: No rent change, but helps market health

Most DEX integrators run cranker bots to keep shards balanced.
...
|--------|---------------|-------|
| Orders per shard | 100-200 | Configurable at market creation |
| Shard account size | ~10KB | Depends on order count |
| Shards per market | 3-20 | Depends on price distribution |
...
| Find shard for price | ~500 per shard | Linear scan |
| Cross-shard match | ~8,000 per fill | Plus order updates |
| Split shard | ~50,000 | Heavy, done via crank |
| Merge shards | ~40,000 | Heavy, done via crank |

### Transaction Limits

With sharding, typical transactions use:
- **~3 CLMM shards** (along the side-specific chain)
- **~3 Limit shards** (along the side-specific chain)
- **1 PropAMM account**
- **~10 other accounts** (market, vaults, mints, etc.)

**Total: ~20 accounts**, well under Solana's 64-account limit.
