...
async function buyTheDip() {
  ...
  console.log(`Bid placed: 100 SOL @ $140`);
}
```
...
async function takeProfit() {
  const tx = await market.placeLimitOrder({
    ...
    amount: SELL_AMOUNT,
    owner: wallet.publicKey,
    expiresAt: Math.floor(Date.now() / 1000) + 86400 * 7, // Expires in 1 week
  });

  await sendAndConfirmTransaction(connection, tx, [wallet]);
  console.log(`Ask placed: 50 SOL @ $160, expires in 1 week`);
}
```
...
const grid: GridLevel[] = [
  // Sells (asks)
  { price: 165_000_000n, amount: 20_000_000_000n, side: Side.Ask },
  { price: 160_000_000n, amount: 20_000_000_000n, side: Side.Ask },
  { price: 155_000_000n, amount: 20_000_000_000n, side: Side.Ask },
  // Buys (bids)
  { price: 145_000_000n, amount: 33_000_000_000n, side: Side.Bid },
  { price: 140_000_000n, amount: 33_000_000_000n, side: Side.Bid },
  { price: 135_000_000n, amount: 34_000_000_000n, side: Side.Bid },
];
...
## Strategy 4: MEV-Protected Large Trades (PropAMM)

**Goal**: Execute large trades without being front-run or sandwiched.

### When to Use PropAMM

- Trade size > $10,000
- Price-sensitive execution
- Any trade where MEV bots might target you
...
async function mevProtectedSwap(inputAmount: bigint, minOutput: bigint) {
  const currentSlot = await getCurrentSlot(connection);

  // Step 1: Place PropAMM order (valid only this slot)
  const placeTx = await market.placePropOrder({
    side: Side.Bid,
    slot: currentSlot,
    price: 155_000_000n,          // Limit price: $155 max
    amount: inputAmount,
    owner: wallet.publicKey,
  });
  ...
  console.log(`MEV-protected swap complete`);
}
```

### Private Submission (Maximum Protection)

For the strongest protection, submit through a private channel:
...
async function privateSwap() {
  ...
  // Submit privately (never hits public mempool)
