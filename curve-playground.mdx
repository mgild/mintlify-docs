...
| Your Position | Skew | What Happens |
|---------------|------|--------------|
| **Long** (holding too much) | Negative (-) | Ask becomes more aggressive → you sell faster |
| **Neutral** (balanced) | Zero (0) | Symmetric bid and ask |
| **Short** (need to buy) | Positive (+) | Bid becomes more aggressive → you buy faster |

**Why it works:**

When you're long, you have **inventory risk** — if price drops, you lose money. You want to reduce that inventory, so you make your ask more attractive:
- More depth on the ask side (more size available to sell)
- Steeper gamma on the ask side (more liquidity near the spread)

The opposite applies when you're short.
...
- Ask gamma: base × 1.4 (steeper, more aggressive)
- Ask depth: base × 1.4 (40% more size)
- Bid gamma: base × 0.6 (flatter, more passive)
- Bid depth: base × 0.6 (40% less size)
...
*"I'm long 500 SOL and need to reduce exposure without panic selling."*

| Parameter | Value | Reasoning |
|-----------|-------|-----------|
| Spread | 25 bps | Stay competitive |
| Range | 8% | Moderate coverage |
| Gamma | 1.5 | Back-load slightly — not desperate |
| Skew | -0.4 | Long, so make asks aggressive |

**Result:** Ask side has 40% more depth and gamma of 2.1 (vs bid's 0.9). You'll sell faster while still maintaining a bid in case price drops.
...
// BID: gamma=1.95, depth=65% of base
// ASK: gamma=1.05, depth=135% of base
```
...
const bidOrder = await market.placeClmmPosition({
  ...
  exponent: 1950,             // 1.95 × 1000
  maxBase: 6_500_000_000n,    // 6.5 SOL (65% of 10)
  maxQuote: 0n,
  side: 'bid',
});
...
const askOrder = await market.placeClmmPosition({
  ...
  exponent: 1050,             // 1.05 × 1000
  maxBase: 13_500_000_000n,   // 13.5 SOL (135% of 10)
  maxQuote: 0n,
  side: 'ask',
});
```
