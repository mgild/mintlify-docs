...
const tx = await Market.placeLimitOrderIx({
  side: Side.Bid,              // Bid = buy, Ask = sell
...
To buy 1 SOL at $150: `price = 150_000_000n`, `amount = 1_000_000_000n`
</Note>
...
const quote = await market.getQuote({
  side: Side.Ask,              // Buying from asks = getting base token
  amount: inputAmount,
});
...
// Execute the swap
const tx = await Market.swapIx({
  side: Side.Ask,              // Buy from asks
  amount: inputAmount,         // How much to spend
  minOut: minOutput,           // Minimum to receive (slippage protection)
  limitPrice: 0n,              // 0 = no price limit (market order)
...
// braid-quickstart.ts
import { Connection, Keypair, PublicKey } from '@solana/web3.js';
import { Market, Side, sendAndConfirmTransaction } from '@braid/client';
...
async function main() {
  ...
  // 3. Place a limit order
  const orderTx = await Market.placeLimitOrderIx({
    side: Side.Bid,
    pMin: 148_000_000n,
    pMax: 150_000_000n,
  ...
  // 4. Execute a swap
  const swapTx = await Market.swapIx({
    side: Side.Ask,
    amount: 100_000_000n,
    minOut: 600_000_000n,
    limitPrice: 0n,
