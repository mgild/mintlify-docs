...
Set a `limitPrice` to only fill at acceptable prices:

```typescript
// Only buy if price is at or below 100
const tx = await Market.swapIx({
  side: Side.Ask,
  amount: 1_000_000n,
  minOut: 0n,
  limitPrice: 100_000_000n,  // Max price willing to pay
  taker: wallet.publicKey,
});
```
